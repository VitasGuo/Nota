...
> 当前为 v0.9.6：引入 whisper.cpp 作为默认本地 ASR 引擎，替代 llama.cpp mtmd 接口（同步 FFI 闪退问题）。whisper.cpp（ggml .bin 模型）专门做 ASR，移动端成熟稳定且质量优；llama.cpp（GGUF）保留用于本地文本 LLM 推理（翻译/纠错/纪要，未来跑 Qwen3 0.6B）；sherpa-onnx 保留作为稳定备选 ASR。引擎优先级：whisper > sherpa > gguf > cloud（默认 whisper）。whisper.cpp 静态链接 ggml + version script 隐藏符号避免与 llama.cpp 的 libggml.so 冲突；C wrapper 简化 FFI（whisper_simple_init/transcribe/free 3 函数）；持久化 worker Isolate 避免同步 FFI 阻塞主线程。继承 v0.9.5 工具型应用按 LLM 任务差异化配置——翻译/纠错（低温度 0.1 + 短输出 + 关闭 thinking）求稳不发散，纪要/笔记整理（中温度 0.4 + 开启 thinking）允许归纳推理。Prompt 加严格格式约束（只输出正文/保留段落/数字专名保留/忠于原文不增删）。LlmTaskRouter 引擎缓存 + 并发去重（`_pendingFutures`）防止重复加载 GB 级模型 OOM；`<think>` 标签过滤防污染纪要/笔记；llama.cpp backend 静态化管理（单实例 dispose 不释放全局 backend，app 退出统一 `disposeBackend()`）；disposeAll 双保险调用（NotaApp.dispose + detached 生命周期）。录音时麦克风 PCM16 流 → VAD 分段 → 本地 ASR 实时转写（用户可选 whisper.cpp/sherpa-onnx/GGUF Qwen3-ASR）→ 文字逐句显示，可选实时翻译到指定语言。含 ASR 引擎、LLM 引擎（云端 SSE + 本地 llama.cpp FFI）、声纹识别、热词词库、数据管理、完整界面 7 分区。继承自 xiaop v1.4.1。
