...
> 当前为 v0.8.1：在 v0.8.0（ModelScope 下载源 + SenseVoice 多语言模型）基础上，修复录音启动"already been listened to"失败、启动失败后状态残留无法重试、GGUF ASR 同步 FFI 阻塞主线程导致闪退三个问题；调整引擎优先级为 sherpa-onnx（SenseVoice > Paraformer）> GGUF ASR > 云端。录音时麦克风 PCM16 流 → VAD 分段 → 本地 ASR 实时转写（SenseVoice 首选 / Qwen3-ASR GGUF / Paraformer / Whisper）→ 文字逐句显示，可选实时翻译。含 ASR 引擎（本地 sherpa-onnx SenseVoice/Paraformer/Whisper + Qwen3-ASR GGUF via llama.cpp mtmd + 云端 OpenAI Whisper API）、LLM 引擎（云端 ai_router_module SSE 流式 + 本地 llama.cpp FFI GGUF 推理）、声纹识别、热词词库、数据管理、完整界面（实时转写录音/转写/笔记列表/笔记详情/热词管理/说话人管理/数据管理/设置 7 分区）。继承自 xiaop v1.4.1 的 ai_router_module 基础设施 + 主题 + 三 tab 导航。
...
- RealtimeAsrEngine（`lib/services/asr/realtime_asr_engine.dart`，v0.6.0，v0.8.0 新增 SherpaRealtimeAsrEngine）：实时 ASR 引擎抽象 + 三个实现
  ...
  - 引擎优先级（v0.8.0）：GGUF ASR > sherpa-onnx ASR（SenseVoice > Paraformer）> 云端 ASR
...
│   │   ├── asr_model_info.dart     # AsrModelInfo + 预置模型清单 + GgufAsrModels（Qwen3-ASR GGUF，v0.6.0）
│   │   ├── asr_model_manager.dart  # ★ 模型下载/激活/删除 + GGUF 双文件管理（v0.6.0）
...
│   │   ├── realtime_asr_engine.dart# ★ RealtimeAsrEngine 抽象 + Local/Cloud 实时实现（v0.6.0）
...
## 核心数据流（实时 ASR 转写，v0.6.0，v0.8.0 新增 SenseVoice）
...
麦克风 PCM16 流 (MicRecorder.startStream, 16kHz 单声道)
  ...
  → 串行 _processQueue (按引擎优先级选择其一):
      ├─ LocalRealtimeAsrEngine: LlamaCppEngine.transcribeAudio (Qwen3-ASR GGUF via mtmd) → 文本
...
> 架构亮点：VAD 同步分段不阻塞音频流，转写异步串行处理；onPartial 未实现（整段推理无 token 级流式），UI 用 onSpeechStart 显示"正在转写..."占位。引擎优先级（v0.8.0）：GGUF ASR > sherpa-onnx ASR（SenseVoice > Paraformer）> 云端 ASR。
...
- **LlmTaskRouter 按功能独立路由（v0.4.0）**：翻译/纪要/笔记/纠错 4 个 LlmTaskType 各自独立配置引擎类型 + 提供商 + 模型 + 高级参数（maxTokens/temperature），SharedPreferences 持久化（key `llm_task_<taskType>`）。流水线执行到某步时读取该功能的配置路由到对应引擎，避免单一模型兼顾所有任务的质量妥协
