// lib/services/asr/whisper_isolate_worker.dart
//
// 持久化 worker Isolate 封装：在独立 Isolate 中执行 whisper.cpp ASR 推理，
// 避免同步 FFI 调用阻塞主 isolate 导致 ANR/crash（traps.md #37/#45）。
//
// 与 [IsolateAsrWorker]（llama.cpp ASR）并行存在的 whisper.cpp 版本：
// - 通信协议完全相同（Map 消息 + SendPort + id 关联）
// - 内部加载 [WhisperEngine] 而非 [LlamaCppEngine]
// - 模型为 whisper.cpp ggml 格式（.bin 单文件），非 GGUF 双文件
//
// 工作流程：
// 1. [spawn] 启动 worker Isolate，在 worker 内加载 whisper.cpp 模型
...
import 'package:nota/services/asr/whisper_engine.dart';

/// 在独立 Isolate 中运行 whisper.cpp ASR 推理的持久化 worker。
///
/// 解决 [WhisperEngine.transcribe] 同步 FFI 阻塞主线程的问题：将模型加载
/// 和推理移到 worker Isolate，主 Isolate 仅通过 SendPort 发送音频段并
/// await 结果，不阻塞 UI 线程。
///
/// 生命周期：[spawn]（加载模型）→ 多次 [transcribe] → [dispose]（释放）。
/// worker Isolate 在 [spawn] 时创建，[dispose] 时销毁，期间复用同一份模型。
class WhisperIsolateWorker {
  ...
  /// 启动 worker Isolate 并加载 whisper.cpp 模型。
  ///
  /// [modelPath] ggml 模型文件路径（如 `ggml-small.bin`）
  ...
  Future<void> spawn(
    ...
    if (_disposed) throw StateError('WhisperIsolateWorker 已释放');
        ...
        if (msg.containsKey('loaded')) {
          if (msg['loaded'] == true) {
            ...
            readyCompleter.completeError(
              StateError('whisper 模型加载失败: ${msg['error']}'),
            );
          }
          return;
        }
  ...
  /// worker Isolate 入口。
  ///
  /// 在独立 Isolate 中运行，接收主 Isolate 的命令并执行 whisper.cpp ASR 推理。
  ...
  /// 处理 'load' 命令：在 worker 内创建 WhisperEngine 并加载模型。
  static void _handleLoad(
    SendPort toMain,
    Map msg,
    void Function(WhisperEngine) setEngine,
  ) {
    try {
      final engine = WhisperEngine(
        language: msg['language'] as String? ?? 'auto',
        nThreads: msg['nThreads'] as int? ?? 4,
      );
      try {
        engine.load(msg['modelPath'] as String);
        setEngine(engine);
  ...
  /// 处理 'transcribe' 命令：调用 engine.transcribe 并返回结果。
  static void _handleTranscribe(
    SendPort toMain,
    Map msg,
    WhisperEngine? Function() getEngine,
  ) {
    ...
    try {
      ...
      final text = engine.transcribe(samples);
